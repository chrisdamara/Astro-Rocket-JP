---
title: "How Just Pixel's Design System Works — Tokens, Colors, and Dark Mode"
description: "Just Pixel uses a three-tier token architecture with OKLCH colors. Change one value and the entire site updates. Here's how it works and how to make it yours."
...
Most themes give you a stylesheet full of hardcoded values. Change the brand colour and you are hunting through dozens of files. Just Pixel works differently — one change propagates everywhere, instantly, in both light and dark mode.
...
Just Pixel uses [OKLCH](https://oklch.com/) for all colour values instead of hex or HSL.
...
Dark mode in Just Pixel is not a separate stylesheet. It is the same token system with different values applied under the `.dark` selector.
...
Just Pixel uses Tailwind CSS v4, which reads CSS custom properties directly. The `brand-500` utility class maps to `--color-brand-500` — no `tailwind.config.js` entry required.
